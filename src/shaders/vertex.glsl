...
void main() {
  ...
  float noise = snoise(vec3(noiseCoord.x + time*2., noiseCoord.y, time * 3.));

  //Remove negative waves
  noise = max (0., noise);
  ...
  for(int i = 0; i < 5; i++) {
    ...
    float noiseFlow = 3. + float(i)*0.3;
    float noiseSpeed = 5. + float(i)* 0.3;

    float noiseSeed = 1. + float(i)*7.;
